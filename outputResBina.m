function outputResBina(u,s,mises,du,sumNode,sumElem,node,elem,shapeChangeRat,mnode,ndim)
% 二进制格式输出结果
node = node+shapeChangeRat*u';

if ndim == 2
    data = [node,u',s,mises,du];
    ZoneType = 3; %'FEQUADRILATERAL';
%     file.varName = {'x','y','ux','uy','sxx','syy','sxy','von-mises'};
    file.varName = {'x','y','ux','uy','sxx','syy','sxy','von-mises','u_x','v_y','u_y','v_x'};
else
    data = [node,u'];
    ZoneType = 5; %'FEBRICK';
